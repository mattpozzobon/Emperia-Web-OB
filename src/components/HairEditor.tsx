...
function renderCompositeThumb(
  ...
  const baseAppearanceId = objectData.outfitAppearances.get(baseDisplayId);
  const baseInternal = baseAppearanceId == null ? -1 : objectData.itemCount + 1 + baseAppearanceId;
  ...
  // Draw hair on top
  if (hairDisplayId >= 0) {
    const hairInternal = objectData.itemCount + objectData.outfitCount + objectData.equipmentCount + 1 + hairDisplayId;
...
function OutfitPicker({ onSelect, onClose }: { onSelect: (displayId: number) => void; onClose: () => void }) {
  ...
  const outfitRange = getCategoryRange('hair');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div ref={containerRef} className="bg-emperia-surface border border-emperia-border rounded-lg shadow-2xl w-[520px] max-h-[80vh] flex flex-col">
        <div className="flex items-center gap-2 px-3 py-2 border-b border-emperia-border shrink-0">
          <h3 className="text-sm font-semibold text-emperia-text flex-1">Select Hair Appearance</h3>
        ...
        <div className="px-3 py-1.5 border-b border-emperia-border shrink-0">
          <div className="relative">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
              placeholder="Search hair appearance ID..." className="w-full pl-7 pr-2 py-1 bg-emperia-bg border border-emperia-border rounded text-xs text-emperia-text" autoFocus />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto p-2">
            ...
            {outfits.map(({ id, displayId }) => {
              ...
              return (
                <button key={id} onClick={() => onSelect(displayId)}
                  ...
                  title={`Hair appearance #${displayId}`}>
          ...
          {outfits.length === 0 && <p className="text-center text-emperia-muted text-xs py-8">No hair appearances found.</p>}
        </div>
      </div>
    </div>
  );
...
function HairPreviewCanvas({ outfitDisplayId }: { outfitDisplayId: number }) {
  ...
  useEffect(() => {
    ...
    // Determine canvas size from the base outfit
    const baseAppearanceId = objectData.outfitAppearances.get(BASE_OUTFIT_DISPLAY_ID);
    const baseInternal = baseAppearanceId == null ? -1 : objectData.itemCount + 1 + baseAppearanceId;
    const baseThing = objectData.things.get(baseInternal);
    const baseFg = baseThing?.frameGroups[0];
    const cellW = baseFg ? baseFg.width * 32 : 64;
    ...
    if (outfitDisplayId >= 0) {
      const hairInternal = objectData.itemCount + objectData.outfitCount + objectData.equipmentCount + 1 + outfitDisplayId;
...
function HairDetail({ hair }: { hair: HairDefinition }) {
  ...
  return (
    <div className="p-3 flex flex-col gap-3">
      {/* Preview */}
      <HairPreviewCanvas outfitDisplayId={hair.appearanceId} />
      <div className="flex items-center justify-between">
        <div>
          <div className="text-sm font-semibold text-emperia-text">{hair.name}</div>
          <div className="text-[10px] text-emperia-muted">Hair #{hair.hairId} · Appearance #{hair.appearanceId}</div>
        </div>
      ...
      <div className="flex gap-2">
        ...
        <div className="flex-1">
          <label className="text-[10px] text-emperia-muted block mb-1">Appearance ID</label>
          <div className="flex gap-1">
            <input type="number" value={hair.appearanceId}
              onChange={(e) => { const v = parseInt(e.target.value, 10); if (!isNaN(v) && v >= 0) update({ appearanceId: v }); }}
      ...
      {/* Validation warnings */}
      {hair.appearanceId < 0 && (
        <p className="text-[10px] text-amber-400 bg-amber-400/10 rounded px-2 py-1">Appearance ID cannot be negative.</p>
      )}
      {!hair.name.trim() && (
      ...
      {showPicker && (
        <OutfitPicker onSelect={(id) => { update({ appearanceId: id }); setShowPicker(false); }} onClose={() => setShowPicker(false)} />
      )}
    </div>
  );
}
...
export function HairEditor() {
  ...
  const filtered = useMemo(() => {
    ...
    return hairDefinitions.filter((h) => {
      if (q && !h.name.toLowerCase().includes(q) && !h.hairId.toString().includes(q) && !h.appearanceId.toString().includes(q)) return false;
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-emperia-muted text-sm p-8 gap-3">
        ...
        <p className="text-[10px] text-emperia-muted/50">
          Open an EOBJ v6 file containing the hair catalog.
        </p>
        ...
        <div className="flex-1 overflow-y-auto">
          {filtered.length === 0 ? (
            ...
            filtered.map((h) => (
              ...
              >
                <HairOutfitThumb outfitDisplayId={h.appearanceId} size={32} showBase />
